...
echo " "
v() {
	DISPLAY=:0.0 wine64 /mnt/vrising/server/DesyncedServer.exe -persistentDataPath $p -serverName "$SERVERNAME" "$override_savename" -logFile "$p/$logfile" "$game_port" "$query_port" 2>&1 &
}
v
